...
In the example below, clicking the map will place a new marker, and refit the map so all markers are visible.
...
You can override the map style by updating the layer values after load with `setPaintProperty`.
...
You can toggle a layer's visibility with `setLayoutProperty`.
